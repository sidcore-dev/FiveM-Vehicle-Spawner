...
- Categorized vehicle list (Sports, SUVs, Off-Road, Motorcycles, Aircraft, Emergency, Utility) — fully editable
- Optional ace-permission gate to restrict who can open the menu
...
| `Config.UsePermissions` | boolean | `false` | If `true`, requires the ace permission below to open the menu. |
| `Config.Permission` | string | `'vehiclespawner.use'` | Ace permission name checked server-side when `UsePermissions` is `true`. |
...
| `Config.MenuTitle` | string | `'Vehicle Spawner v2.0'` | Title shown at the top of the menu. |
| `Config.NotifyDuration` | number | `5000` | How long (ms) mythic_notify alerts stay on screen. |
| `Config.Vehicles` | table | see below | Categories and vehicles shown in the menu. |

### Enabling permissions

Set `Config.UsePermissions = true`, then grant the ace permission in `server.cfg`:
...
### Editing the vehicle list

`Config.Vehicles` is a table of categories, each containing a list of `{ label, model }` entries:
...
- Add, remove, or rename categories and vehicles freely — no other code changes needed.
